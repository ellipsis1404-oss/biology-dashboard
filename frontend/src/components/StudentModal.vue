            ...
            <div v-else-if="studentData">
                ...
                <div class="modal-body">
                    <div class="section ai-section">
                        <div class="ai-header">
                            <h3>AI Generated Summary</h3><button @click="generateAiSummary" :disabled="isGeneratingSummary">{{ isGeneratingSummary ? 'Generating...' : '✨ Generate Summary' }}</button>
                        </div>
                        <div v-if="isGeneratingSummary" class="summary-box">Thinking...</div>
                        <div v-if="aiSummary" class="summary-box"><p style="white-space: pre-wrap;">{{ aiSummary }}</p></div>
                    </div>
                    <div class="section">
                        <h3>Standards Mastery</h3>
                        <ul class="standards-list">
                            <li v-for="standard in studentData.standards_performance" :key="standard.id">
                                <div class="standard-info"><strong>{{ standard.code }}</strong> - {{ standard.description }}<em>Unit: {{ standard.unit }}</em></div>
                                <div class="standard-score">{{ Math.round(standard.percentage) }}%</div>
                            </li>
                        </ul>
                    </div>
                    <div class="section">
                        ...
                        <form @submit.prevent="handleAddComment" class="comment-form">
                            <textarea v-model="newCommentText" placeholder="Add a new comment..." required></textarea><button type="submit" :disabled="isSubmitting">{{ isSubmitting ? 'Saving...' : 'Add Comment' }}</button>
                        </form>
                        <div v-if="studentData.comments.length > 0">
                            <ul class="comments-list">
                                <li v-for="comment in studentData.comments" :key="comment.id">
                                    <p>{{ comment.text }}</p><span>{{ new Date(comment.created_at).toLocaleDateString() }}</span>
                                </li>
                            </ul>
                        </div>
...
.ai-section{background-color:rgba(0,0,0,.2);padding:1rem 1.5rem;border-radius:8px;margin-top:0}.ai-header{display:flex;justify-content:space-between;align-items:center}.ai-header button{background-color:#8e44ad;border:none;padding:8px 15px;border-radius:5px;cursor:pointer;color:white;font-weight:bold}.ai-header button:disabled{background-color:#9b59b6;cursor:not-allowed}.summary-box{background-color:#2c3e50;margin-top:1rem;padding:1rem;border-radius:5px;color:#bdc3c7}
