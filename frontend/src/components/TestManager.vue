...
// --- CHANGED: Initialize standards as an empty array [] ---
...
async function fetchTests(classId) {
  ...
  try {
    const response = await axios.get(`http://127.0.0.1:8000/api/tests/?search=${classId}`);
...
async function handleCreateTest() {
  ...
  try {
    await axios.post('http://127.0.0.1:8000/api/tests/', {
      ...newTestForm.value,
...
async function handleAddQuestion() {
  ...
  try {
    ...
    // --- CHANGED: Reset standards to an empty array [] ---
    newQuestionForm.value = { question_text: '', max_mark: 1, standards: [] };
    // We need to refetch the *entire test list* to get the updated nested questions
    const currentlySelectedTestId = selectedTest.value.id;
    await fetchTests(selectedClassId.value);
    // After refetching, re-select the test to update the view
...
async function handleSaveScores() {
  ...
  try {
    await axios.post(`http://127.0.0.1:8000/api/tests/${selectedTest.value.id}/bulk_score_entry/`, {
      scores: scores.value
    });
...
<template>
  <div>
    <!-- Step 1: Select a Class (No changes here) -->
    ...
    <div v-if="selectedClassId">
      <!-- Step 2: Create or Select a Test (No changes here) -->
      <div class="step-container">
        ...
        <div class="form-grid">
          <form @submit.prevent="handleCreateTest" class="form-card">
            ...
            <input type="text" v-model="newTestForm.title" placeholder="Test Title (e.g., Unit 1 Exam)" required />
            <input type="date" v-model="newTestForm.date_administered" required />
      ...
      <div v-if="selectedTest">
        <!-- Step 3: Add Questions to the Selected Test -->
        <div class="step-container">
          <h3>Step 3: Add Questions to "{{ selectedTest.title }}"</h3>
          <div class="questions-section">
            <ul class="question-list">
              <li v-for="q in selectedTestDetails?.questions" :key="q.id">
                {{ q.question_text }} ({{ q.max_mark }} marks)
              </li>
            ...
            <form @submit.prevent="handleAddQuestion" class="form-card">
              ...
              <input type="text" v-model="newQuestionForm.question_text" placeholder="Question Text" required />
              <input type="number" v-model="newQuestionForm.max_mark" placeholder="Max Mark" min="1" required />
              
              <!-- --- CHANGED: Added the 'multiple' attribute to the select element --- -->
              <label>Associated Standards (Hold Ctrl/Cmd to select multiple)</label>
        ...
        <!-- Step 4: Bulk Score Entry Grid (No changes here) -->
        <div class="step-container">
          ...
          <div v-if="students.length && selectedTestDetails?.questions.length" class="score-grid-container">
            <table class="score-table">
              <thead>
                <tr>
                  <th>Student Name</th>
                  <th v-for="q in selectedTestDetails.questions" :key="q.id">
                    {{ q.question_text }} (Max: {{ q.max_mark }})
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="student in students" :key="student.id">
                  ...
                  <td v-for="q in selectedTestDetails.questions" :key="q.id">
                    <input type="number" v-model="scores[student.id][q.id]" :max="q.max_mark" min="0" />
                  </td>
                </tr>
              </tbody>
            </table>
            <button @click="handleSaveScores" class="save-scores-btn">Save All Scores</button>
          </div>
...
<style scoped>
/* All styles from before are the same */
