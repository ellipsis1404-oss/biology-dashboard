...
const newTestForm = ref({ title: '', date_administered: new 
  ...
  if (newTest && newTest.questions) {
    ...
    students.value.forEach(student => {
      ...
      newTest.questions.forEach(question => {
        newScores[student.id][question.id] = '';
      });
    });
    ...
    try {
      ...
      response.data.forEach(score => {
        if (scores.value[score.student] && newScores[score.student][score.question] !== undefined) {
          scores.value[score.student][score.question] = score.mark_awarded;
        }
      });
...
async function fetchTestDetails(testId) {
    ...
    try {
        ...
        selectedTestDetails.value = response.data;
...
<template>
  <div class="main-layout">
    ...
    <v-card class="mb-6">
      ...
      <v-card-text>
        <v-select v-model="selectedClassId" :items="classes" item-title="name" item-value="id" label="-- Choose a class --" dense variant="outlined" hide-details></v-select>
      </v-card-text>
    </v-card>

    <!-- Step 2 appears after a class is selected -->
    <v-card v-if="selectedClassId" class="mb-6">
      ...
      <v-row class="pa-4">
        ...
        <v-col cols="12" md="6">
          <v-select v-model="selectedTestId" :items="tests" item-title="title" item-value="id" label="Select Existing Test" dense variant="outlined" :loading="isLoadingTests" hide-details></v-select>
        </v-col>
      </v-row>
    </v-card>
        ...
        <v-row class="pa-4">
          <v-col cols="12" md="7">
            <v-list lines="two" border>
              <v-list-subheader>Question List</v-list-subheader>
          ...
          <v-col cols="12" md="5">
            <v-form @submit.prevent="handleAddQuestion">
                <v-text-field label="Question Number" v-model.number="newQuestionForm.question_number" type="number" min="1" required variant="outlined" class="mb-2"></v-text-field>
                <v-text-field label="Question Text" v-model="newQuestionForm.question_text" required variant="outlined" class="mb-2"></v-text-field>
                <v-text-field label="Max Mark" v-model.number="newQuestionForm.max_mark" type="number" min="1" required variant="outlined" class="mb-2"></v-text-field>
                <v-autocomplete label="Associated Standards" v-model="newQuestionForm.standards" :items="standards" item-title="code" item-value="id" multiple chips variant="outlined" class="mb-2"></v-autocomplete>
              ...
              <tbody>
                <tr v-for="student in students" :key="student.id">
                  ...
                  <td v-for="q in selectedTestDetails.questions" :key="q.id">
                    <v-text-field v-model.number="scores[student.id][q.id]" :max="q.max_mark" min="0" type="number" density="compact" hide-details variant="outlined"></v-text-field>
                  </td>
                </tr>
              </tbody>
            </v-table>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12"><v-autocomplete label="Associated Standards" v-model="questionToEdit.standards" :items="standards" item-title="code" item-value="id" multiple chips></v-autocomplete></v-col>
            </v-row>
          </v-container>
        </v-card-text>
        <v-card-actions>
...
.main-layout {
  ...
  gap: 1.5rem; /* Adds space between the v-card steps */
}
